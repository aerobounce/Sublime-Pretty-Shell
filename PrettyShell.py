...
def get_plugin_settings():
    return load_settings(SETTINGS_FILENAME)
...
class PrettyShell:
    ...
    @staticmethod
    def construct_shell_command():
        settings = get_plugin_settings()

        simplify = settings.get("simplify")
        minify = settings.get("minify")
        language = settings.get("language")
        indent = settings.get("indent")
        binop = settings.get("binop")
        switchcase = settings.get("switchcase")
        rediop = settings.get("rediop")
        align = settings.get("align")
        fnbrace = settings.get("fnbrace")

        shell_command = settings.get("shfmt_bin_path")
        if simplify:
            shell_command += " -s"
        if minify:
            shell_command += " -mn"
        if language:
            shell_command += ' -ln "{}"'.format(language)
        if indent:
            shell_command += " -i {}".format(indent)
        if binop:
            shell_command += " -bn"
        if switchcase:
            shell_command += " -ci"
    ...
    def execute_format(cls, view: View, edit: Edit):
        settings = get_plugin_settings()
        shell_command = cls.construct_shell_command()
        ...
        with Popen(shell_command, cwd=shell_cwd, shell=True, stdin=PIPE, stdout=PIPE, stderr=PIPE) as popen:
        ...
        print("[Pretty Shell] Popen:", shell_command)
        ...
        if settings.get("show_error_inline") and error_point:
            cls.update_phantoms(view, stderr, error_point)

        # Scroll to the syntax error point
        if settings.get("scroll_to_error_point") and error_point:
            view.sel().clear()
